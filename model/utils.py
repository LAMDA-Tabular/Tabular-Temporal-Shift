...
def get_deep_args():  
    ...
    parser.add_argument('--model_type', type=str, 
                        ...
                        choices=['mlp', 'resnet', 'ftt', 'node', 'autoint',
                                 ...
                                 'modernNCA_linear', 'modernNCA_causal', 'modernNCA_causal_offset',
                                 'modernNCA_periodic', 'modernNCA_online', 'modernNCA_weighted',
                                 'mlp_temp_plr', 'mlp_plr_temp_plr', 'modernNCA_temp_plr', 
                                 'mlp_temporal', 'mlp_plr_temporal', 'ftt_temporal', 'dcn2_temporal', 
                                 'snn_temporal', 'tabr_temporal', 'modernNCA_temporal', 'tabm_temporal',
                                 ])
...
def tune_hyper_parameters(args, opt_space, train_val_data, info, validate_option='holdout_last'):
    ...
    def objective(trial):
        ...
        if args.model_type in ['tabm', 'tabm_temporal']:
            config['model'].setdefault('arch_type', 'tabm')
            config['model']["num_embeddings"].setdefault('type', 'PLREmbeddings')
            config['model']["num_embeddings"].setdefault('lite', True)
            config['model']['backbone'].setdefault('type', 'MLP')
        
        if args.model_type in ['mlp_plr', 'mlp_plr_temporal', 'mlp_plr_temp_plr']:
            config['model']["num_embeddings"].setdefault('type', 'PLREmbeddings')
            config['model']["num_embeddings"].setdefault('lite', True)
        ...
        if args.model_type in ['modernNCA', 'modernNCA_linear', 'modernNCA_online', 'modernNCA_causal', 'modernNCA_causal_offset', 'modernNCA_periodic', 'modernNCA_weighted', 'modernNCA_temporal', 'modernNCA_temp_plr']:
            config['model']["num_embeddings"].setdefault('type', 'PLREmbeddings')
            config['model']["num_embeddings"].setdefault('lite', True)
...
def get_method(model):
    ...
    elif model == 'modernNCA_linear':
        from model.methods.modernNCA_linear import ModernNCA_LinearMethod
        return ModernNCA_LinearMethod
    # elif model == 'modernNCA_online':
    #     from model.methods.modernNCA_online import ModernNCA_OnlineMethod
    #     return ModernNCA_OnlineMethod
    elif model == 'modernNCA_causal':
        from model.methods.modernNCA_causal import ModernNCA_CausalMethod
        return ModernNCA_CausalMethod
    # elif model == 'modernNCA_causal_offset':
    #     from model.methods.modernNCA_causal import ModernNCA_Causal_OffsetMethod
    #     return ModernNCA_Causal_OffsetMethod
    elif model == 'modernNCA_periodic':
        from model.methods.modernNCA_periodic import ModernNCA_PeriodicMethod
        return ModernNCA_PeriodicMethod
    elif model == 'modernNCA_weighted':
        from model.methods.modernNCA_weighted import ModernNCA_WeightedMethod
        return ModernNCA_WeightedMethod
    
    elif model == 'mlp_temp_plr':
        from model.methods.mlp_temp_plr import MLP_Temp_PLRMethod
        return MLP_Temp_PLRMethod
    elif model == 'mlp_plr_temp_plr':
        from model.methods.mlp_plr_temp_plr import MLP_PLR_Temp_PLRMethod
        return MLP_PLR_Temp_PLRMethod
    elif model == 'modernNCA_temp_plr':
        from model.methods.modernNCA_temp_plr import ModernNCA_Temp_PLRMethod
        return ModernNCA_Temp_PLRMethod
